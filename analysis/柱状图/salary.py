...
d7 = [x for x in df['salary'] if 60 < x <= 70]
number.append(len(d7))
message.append("60-70k")
d8 = [x for x in df['salary'] if 70 < x <= 80]
number.append(len(d8))
message.append("70-80k")
print(number,message)


mpl.rcParams['font.sans-serif'] = ['SimHei']
mpl.rcParams['axes.unicode_minus'] = False
plt.bar(message, number, hatch='/',alpha=0.8)
plt.xlabel('薪水范围')
plt.ylabel('职位数(个)')
plt.title("薪水分布")
plt.savefig("../../tmp/柱状图/salaryBar.png")
plt.show()
